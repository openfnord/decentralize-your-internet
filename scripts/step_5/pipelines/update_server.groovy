...
// replace your_credentials_id_here from the script with the ID of your just created access data → "manage jenkins" → "Credentials"
//
...
pipeline {
    ...
    stages {
        stage('update') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'your_credentials_id_here', usernameVariable: 'SUDO_USER', passwordVariable: 'SUDO_PW')]) {
